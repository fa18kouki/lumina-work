...
interface OwnerInviteEmailProps {
  url: string;
}

/**
 * admin による Owner 招待メール。
 * 送信主体は `src/server/auth/owner-email.ts` の `sendOwnerInviteEmail` (Resend SDK)。
 */
export function OwnerInviteEmail({ url }: OwnerInviteEmailProps) {
  return (
    <NotificationLayout
...
export function buildOwnerInviteSubject(): string {
  return `${SERVICE_NAME} オーナー招待のお知らせ`;
}

export function buildOwnerInviteText(url: string): string {
