    ...
    .query(async ({ ctx, input }) => {
      const stores = await ctx.prisma.store.findMany({
        where: {
          ...
          ...(input.hasDailyPay && { dailyPayType: { not: "NONE" } }),
          ...(input.hasNoQuota && { hasQuota: false }),
          ...(input.hasDormitory && { hasDormitory: true }),
          ...(input.hasNursery && { hasNursery: true }),
        },
        take: input.limit + 1,
